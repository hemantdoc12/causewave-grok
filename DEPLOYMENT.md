...
| WebSite | Homepage | Name, URL (no SearchAction — there is no /search route) |
...
- Output: `sitemap-index.xml` → `sitemap-0.xml` (each `<url>` includes `<lastmod>`)
- Alias: `dist/sitemap.xml` is a copy of `sitemap-index.xml` (so `/sitemap.xml` is valid XML, not a 404)
- Canonical URL to submit in GSC: `https://causewave.in/sitemap-index.xml`
- Do **not** submit page URLs as sitemaps (e.g. `/insights/csr-sector-ticket-league/` is HTML — GSC will report “Sitemap is HTML”)
...
### HSTS
GitHub Pages on a custom domain does **not** send `Strict-Transport-Security`. Enforce HTTPS is on in Pages settings (HTTP→HTTPS redirect). A true HSTS header would need a CDN in front (e.g. Cloudflare). Do not fake the header in HTML.
...
- Sitemap submitted: **only** `https://causewave.in/sitemap-index.xml`
- If GSC lists another row such as `https://causewave.in/insights/csr-sector-ticket-league/` with error **Sitemap is HTML**, remove that submission. It is a normal article URL, not a sitemap.
- Ghost 404 `https://causewave.in/search?q={search_term_string}`: leftover from an old `WebSite` `SearchAction`. Current homepage JSON-LD has no SearchAction; `robots.txt` disallows `/search`. Use URL Inspection → Validate fix, then ignore once it drops.
